...
int btd_adapter_stop(struct btd_adapter *adapter)
{
	...
	if (adapter->discovering){
		adapter->discovering = FALSE; //modified by ltz for alway discovering
		emit_property_changed(connection, adapter->path,
					...
					uint8_t *data, uint8_t data_len)
{
	...
	if (eir_data.name != NULL/* && eir_data.name_complete*/) //modified by ltz for shorted name
...
void adapter_mode_changed(struct btd_adapter *adapter, uint8_t scan_mode)
{
	...
	if (adapter->scan_mode == scan_mode) {
		...
		/* We may reset scan_mode already in btd_adapter_stop(), hence comes to here. */
		set_mode_complete(adapter);
#endif
		return;
	}
