...
GlyphData Font::glyphDataForCharacter(UChar32 c, bool mirror, FontDataVariant variant) const
{
    ...
    // After system language changes GlyphPageTreeNode::resetRoots() will be
    // called to clear the GlyphPageTreeNode. But the node pointer may still
    // be hold by m_fontList. We add some tricky method to judge if the
    // node pointer is still alive before accessing it.
    if (!GlyphPageTreeNode::isAlive(node)) {
