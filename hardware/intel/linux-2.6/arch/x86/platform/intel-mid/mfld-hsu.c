...
static struct mfld_hsu_info default_hsu_info[] = {
	[0] = {
		...
		.name = "hsu0",
		.wake_gpio = 78,
		.btwake_gpio = 114,/*broadcom Btwake gpio num*/
		.cts_gpio = 96+28,
		.cts_alt = 1,
		.rts_gpio = 96+29,
		.rts_alt = 1,
...
void intel_mid_hsu_suspend(int port)
{
	...
	struct mfld_hsu_info *info = platform_hsu_info + port;
	if(port == 0)
		dev_err(info->dev, "@@@@ TTY MFD0 runtime suspend enter\n");
	if (info->rts_gpio) {
		gpio_direction_output(info->rts_gpio, 1);
		lnw_gpio_set_alt(info->rts_gpio, LNW_GPIO);
	}
	...
	if (info->wake_gpio) {
		dev_err(info->dev, " @@@@@ register wakeup irq %d\n",info->wake_gpio);	
...
void intel_mid_hsu_resume(int port)
{
	struct mfld_hsu_info *info = platform_hsu_info + port;
	if(port == 0)
		dev_err(info->dev, "@@@@ TTY MFD0 runtime resume enter\n");
