...
/* YKB thermal sensor list */
static struct intel_mid_thermal_sensor ykb_sensors[] = {
	{
		.name = SKIN0_NAME,
		.index = 0,
		.slope = 879,
...
/* slp thermal sensor list */
static struct intel_mid_thermal_sensor slp_sensors[] = {
	{
		.name = SKIN0_NAME,
		.index = 0,
		.slope = 950,
...
{
        /* Disable the SPID detection logic as SPID is not correctly set
	if (INTEL_MID_BOARD(1, PHONE, CLVTP) ||
		...
		pr_info("msic_thermal_platform_data: YKB board detected\n");
		return &pdata[ykb_thermal];
	} else if (INTEL_MID_BOARD(2, TABLET, MFLD, SLP, ENG) ||
			(INTEL_MID_BOARD(2, TABLET, MFLD, SLP, PRO))) {
		pr_info("msic_thermal_platform_data: SLP board detected\n");
		return &pdata[slp_thermal];
	} else {
		pr_info("msic_thermal_platform_data: MFLD board detected\n");
		return &pdata[mfld_thermal];
	}
	} */
        /* Hard code the logic to YKB */
	pr_info("msic_thermal_platform_data: YKB board detected\n");
	return &pdata[ykb_thermal];
}
