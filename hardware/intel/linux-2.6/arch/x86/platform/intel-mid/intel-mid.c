...
int sfi_mrtc_num;
//start [Du Yichun, 20120627, for hw version id, add]
char hw_version[20] = "";
EXPORT_SYMBOL_GPL(hw_version);
//end
...
struct kobject * intel_get_properties(void)
{
...
//start [Du Yichun, 20130618, for hw version id, add]
static int get_intel_hw_version(void)
{
#define BIT_0    0x01
#define GPIO0LV0  0x050
    int i;
	uint8_t reg_val;
	uint8_t tmp_val= 0x00;
	uint16_t reg_addr[8];
    if (board_id != 0xff) {
   		pr_info("Get board_id[%02x] from firmware\n", board_id);
         } else {
         	for (i=0; i<8; i++) {
			reg_addr[i]=GPIO0LV0 + i;
			intel_scu_ipc_ioread8(reg_addr[i], &reg_val);
			tmp_val = tmp_val |(( reg_val & BIT_0) << (7-i) );
			pr_debug("reg_addr[%02x]=[%02x] pin_status[%02x]\n", reg_addr[i], reg_val, (reg_val & BIT_0));
         	}
		board_id = tmp_val;
		pr_info("Get board_id[%02x] form MSIC\n", board_id);
   	}

   	switch (board_id) {  	  
   	case HW_BOARD_PVT:  strcpy(hw_version, "PVT");   break; 	
  	case HW_BOARD_P2:   strcpy(hw_version, "P1");    break;
 	case HW_BOARD_P3:   strcpy(hw_version, "P2");    break;
 	case HW_BOARD_PCR:  strcpy(hw_version, "PCR");   break;
   	default:            
    	  strcpy(hw_version, "Invalid");
     	  pr_err("ERR:  hw_version is invalid, invalid version[%02x]\n",board_id);
		  return -1;
		}
  	return 0;
}

static ssize_t hw_version_show(struct device *dev,struct device_attribute *attr,char *buf)
{
	  return sprintf(buf, "%s\n", hw_version);
}

static struct kobj_attribute hw_version_attr = {
	.attr = {
		.name = "hw_version",
		.mode = S_IRUGO,
	},
...
static int __init intel_mid_boardID(void)
{
	if (get_intel_hw_version() < 0)
		pr_err("get hw_version invalid!\n");
	intel_mid_create_property(&hw_version_attr.attr);
	return 0;	
}
device_initcall(intel_mid_boardID);
//end

//dfu add for bq27425
static struct i2c_board_info __initdata bq27425_info[] = {
    {
	    I2C_BOARD_INFO("bq27425_battery", 0x55),
	},
};
//end

//pjn add for gps
static struct platform_device gps_bcm4751_device = {
    .name = "agpsgpio",
    .id = -1,
};
//end
...
static int __init intel_mid_platform_init(void)
{
	...
	printk("===try to register fuel gauge -- bq27425.\n");
	i2c_register_board_info(1, bq27425_info,1);
	//pjn add for gps 
	platform_device_register(&gps_bcm4751_device);
	//end
	return 0;
}
