...
static int rfkill_gpio_set_power(void *data, bool blocked)
{
	...
	printk("langwenlong set power state to %d\n",blocked);
	//blocked ==1 means rfkill state ==0;blocked ==0 means rfkill state ==1
	if (blocked) {
...
static int rfkill_gpio_probe(struct platform_device *pdev)
{
	...
	printk("rfkill_gpio_probe pdate->name = %s pdata->reset_gpio = %d  pdata->shutdown_gpio =%d\n",pdata->name,pdata->reset_gpio,pdata->shutdown_gpio);
	...
	rfkill->rfkill_dev = rfkill_alloc(pdata->name, &pdev->dev, RFKILL_TYPE_BLUETOOTH,
