...
static const char *sn95031_ihfmux_cfg_text[] = {"Disable", "Enable"};
static const struct soc_enum sn95031_ihfmux_cfg_enum =
	SOC_ENUM_SINGLE(0, 0, 2, sn95031_ihfmux_cfg_text);
static const struct snd_kcontrol_new sn95031_ihfmux_control =
	SOC_DAPM_ENUM_VIRT("Speaker Playback Mux", sn95031_ihfmux_cfg_enum);
...
static const struct snd_soc_dapm_widget sn95031_dapm_widgets[] = {
	...
	SND_SOC_DAPM_OUTPUT("Virt IHFOUT"),
	...
	SND_SOC_DAPM_AIF_IN("PCM2_IN", "Headset", 0,
			SND_SOC_NOPM, 0, 0),
	SND_SOC_DAPM_AIF_IN("PCM2_IN_SPK", "Speaker", 0,
			SND_SOC_NOPM, 0, 0),
	...
	SND_SOC_DAPM_VIRT_MUX("Speaker Playback Mux",
			SND_SOC_NOPM, 0, 0, &sn95031_ihfmux_control),
	/* Dummy widget to trigger VAUDA on/off */
	SND_SOC_DAPM_MICBIAS("VolumeBias", SND_SOC_NOPM, 0, 0),
...
static const struct snd_soc_dapm_route sn95031_audio_map[] = {
	...
	{ "Speaker Mux Playback Route", "Music", "Speaker Playback Mux"},
	{ "Speaker Mux Playback Route", "Music", "Speaker Playback Mux"},
	{ "Speaker Mux Playback Route", "Voice", "Headset Left Filter"},
	{ "Speaker Mux Playback Route", "Voice", "Headset Right Filter"},
	{ "Speaker Playback Mux", "Enable", "IHFDAC Left"},
	{ "Speaker Playback Mux", "Enable", "IHFDAC Right"},
	{ "IHFDAC Left", "NULL", "Speaker Left Filter"},
	{ "IHFDAC Right", "NULL", "Speaker Right Filter"},
	{ "Virt IHFOUT", NULL, "Speaker Left Filter"},
	{ "Virt IHFOUT", NULL, "Speaker Right Filter"},
	{ "Speaker Left Filter", NULL, "PCM2_IN_SPK"},
	{ "Speaker Right Filter", NULL, "PCM2_IN_SPK"},
...
#ifdef CODEC_DEBUG
static ssize_t msic_all_store(struct device *dev,struct device_attribute *attr, const char *buffer, size_t count)
{
	...
	sscanf(buffer, "%c 0x%x 0x%x", &cmd, &reg, &val);
	printk("langwenlong %c   0x%x    0x%x\n",cmd,reg,val);
	switch(cmd)
		{
			case 'r':
				ret = intel_scu_ipc_ioread8(reg, &val1);
				if (ret)
					pr_err("read of %x failed, err %d\n", reg, ret);
				else
					printk("reg[%3x] = 0x%2x\n",reg,val1);
			...
			case 'w':	
				...
				if (ret)
					pr_err("write of %x failed, err %d\n", reg, ret);
				ret = intel_scu_ipc_ioread8(reg, &val1);
				printk("write 0x%x to 0x[%3x] , read back 0x%02x\n",val,reg,val1);
				break;
...
static ssize_t msic_all_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	u8 value;
	int len = 0;
	int reg;
	int ret;
	for(reg=0x240;reg<=0x27c;reg++) {
		ret = intel_scu_ipc_ioread8(reg, &value);
		if (ret)
			pr_err("read of %x failed, err %d\n", reg, ret);
		else
		len += sprintf(buf + len, "[%03x]=0x%02x \n", reg,value);
	
	}
	return len;
}
static DEVICE_ATTR(msic, 0664, msic_all_show, msic_all_store);
static struct attribute *msic_attributes[] = {
	&dev_attr_msic.attr,
};
static struct attribute_group msic_properties_attr_group = {
	.attrs = msic_attributes,
};
...
static int __init sn95031_init(void)
{
	...
	int ret;
	properties_kobj = kobject_create_and_add("msic_properties", NULL);
	if (properties_kobj)
		ret = sysfs_create_group(properties_kobj,&msic_properties_attr_group);
	if (!properties_kobj || ret)
		printk(KERN_ERR "failed to create msci_properties\n");
#endif	
