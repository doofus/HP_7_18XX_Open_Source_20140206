...
static void sdhci_status_notify_cb(int card_present, void *dev_id)
{
	...
	pr_err("%s:SDIO card detect status: %d -> %d ",__func__,cd_present, card_present);
	//mmc_detect_change(host->mmc,0);
	mmc_detect_change(host->mmc,msecs_to_jiffies(200));
	printk("debug mmc detect change done\n");
	cd_present = card_present;
}
#endif
...
#if 1
//#ifdef CONFIG_BCMDHD
	if(pdev->device == PCI_DEVICE_ID_INTEL_MFD_SDIO1 || pdev->device == PCI_DEVICE_ID_INTEL_CLV_SDIO1){
		pr_err("%s:@@@@@ SDIO device register callback function\n",__func__);
		host->mmc->pm_flags |= MMC_PM_IGNORE_PM_NOTIFY; /* wang.junxian2@byd.com */
		brcm_wifi_status_register(sdhci_status_notify_cb,host);
	}
#endif
	slot = sdhci_priv(host);
...
//pjn add create file node 121113
int sd_state = 0;

static struct kobject *sd_kobject = NULL;
 
static ssize_t attr_sd_state_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", sd_state);
}
static DEVICE_ATTR(sd_state, 0444, attr_sd_state_show, NULL);

static int sd_sys_init(void)
{
	int ret;

	sd_kobject = kobject_create_and_add("sd", NULL);
	if (sd_kobject == NULL)
	{
		printk(KERN_ERR "attr_sd: subsystem_register failed\n");
		ret = -ENOMEM;
	}

	ret = sysfs_create_file(sd_kobject, &dev_attr_sd_state.attr);

	if (ret) 
	{
		printk(KERN_ERR "failed to register sysfs\n");
		return ret;
	}
	return 0;
}
