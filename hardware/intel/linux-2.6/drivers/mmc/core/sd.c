...
extern int sd_state;//pjn 121113
	...
	struct mmc_card *oldcard)
{
	...
	if (rocr & SD_ROCR_S18A) {
		...
		if ((host->caps & MMC_CAP_4_BIT_DATA) &&
			...
			if (err) {
				/*
				 * Workaround: some special sd card cannot
				 * work properly in 50Mhz after switch to
				 * high speed mode. If in this case, reduce
				 * the clock frequence.
				 */
				pr_err("%s %s: set_bus_width failed, retry\n",
						__func__, mmc_hostname(host));
				mmc_set_clock(host, card->csd.max_dtr);
				if (mmc_app_set_bus_width(card,
							MMC_BUS_WIDTH_4))
					goto free_card;
			}
