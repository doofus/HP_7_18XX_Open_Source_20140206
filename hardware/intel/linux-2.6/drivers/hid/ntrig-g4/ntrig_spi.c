...
static void spi_set_output_enable(struct ntrig_spi_privdata *pdata, bool enable)
{
	if (pdata->oe_gpio >= 0) {
		...
		printk("ntrig spi_set_output_enable pin:%d val:%d\n", pdata->oe_gpio, val);
	}
}
...
static void spi_process_message(struct ntrig_spi_privdata *privdata)
{
		...
		if (msg->function == LOWMSG_FUNCTION_MT_REPORT) {
			...
			if (msg->flags & 0x80) {
				...
				if (contactCount > MAX_MT_FINGERS_G4) {
					ntrig_err("invalid g4 mt report, "
						"too many contactCount: %d\n", contactCount);
			...
			g4_WriteHIDNTRIG(mr);
				...
				if (contactCount > MAX_MT_FINGERS_G3) {
					ntrig_err("%s: invalid g3 mt report, "
						"too many fingers: %d\n",
						__func__, contactCount);
					return;
				}
			...
			for (i = 0; i < contactCount; i++) {
				...
				finger->palm = !((fingers[i].flags & 0x04) >>
					2);
					/*1=touch valid, 0=palm detected*/
				/*ntrig_err("%d x:%d y:%d dx:%d dy:%d fingerIndex:%d\n",i,fingers[i].x,fingers[i].y,fingers[i].dx,fingers[i].dy,fingers[i].fingerIndex);*/
			}
						spi_cntrs_list[CNTR_MULTITOUCH].count++;
			/* call the dispatcher to deliver the message */
			g4_WriteHIDNTRIG(mr);
	...
	case LOWMSG_CHANNEL_MAINT_REPLY:
	{
		...
		if ((size + sizeof(u16)) > kfifo_size(&privdata->raw_fifo)) {
			...
			ntrig_err("%s: packet too large to put in buffer "
...
#define FIRST_HB_MESSAGE 0x01
		...
		if (data[6] == 0x20 && data[7] == 0x1)
			printk(KERN_DEBUG "heartBit, data[5]=0x%X\n",
				(int)data[5]);
		if (data[5] == 0x82 && data[6] == 0x20 && data[7] == 0x1 &&
		   (privdata->filter_HB || (data[67] == FIRST_HB_MESSAGE &&
				data[68] == 0))) {
			printk(KERN_DEBUG "%s: Discarding heartbeat\n",
				__func__);
			break;
		}
		...
		if (avail < size) {
			...
			ntrig_err("%s: raw receive buffer is full, discarding "
				"messages\n", __func__);
			do {
...
static int ntrig_get_irq_value(void)
{
	...
	if (gafr)
		...
		printk(KERN_ERR "ntrig_get_irq_value(): ERROR, ioremap FAILED!\n");

	iounmap(gafr);

	/*printk(KERN_DEBUG "ntrig_get_irq_value:%d\n",((gafr_value >> 30) & 0x01));*/
...
static irqreturn_t spi_irq(int irq, void *dev_id)
{
	...
	/*printk(KERN_DEBUG "ntrig spi_irq start irq:%d\n",ntrig_get_irq_value());*/
#if 0
	...
	while (1)
	{
		...
		if (err) {
			printk("%s: spi_transfer failure %d, bailing out\n",
							__FUNCTION__, err);
			...
			if (ntrig_get_irq_value() == 0) {
			    ...
			    if (failCounter > 5) {
					int err;
					failCounter = 0;
					printk("reset to ntrig\n");

					err = gpio_direction_output(176, 0);
					if (err) {
						printk(KERN_ERR "Setting gpio "
							"176 to 0 failed\n");
					}
					msleep(500);
...
#endif
		/*printk("%s: state machine %s idle, gpio is %s\n",
		 *	__FUNCTION__,(sm_idle ? "is" : "not"),
		 *	(irq_high ? "high" : "low"));*/
		if (sm_idle && (!irq_high))
			break;
	}
/*printk(KERN_DEBUG "ntrig spi_irq end irq:%d\n",ntrig_get_irq_value());*/
	return IRQ_HANDLED;
}
...
static int spi_write_raw(void *dev, const char *buf, short msg_len)
{
	...
	case LOWMSG_REQUEST_NCP_DFU:
	{
		...
		if (request == LOWMSG_REQUEST_NCP_DFU && buf[6] == 0x07 &&
			 ...
			 */
			ntrig_dbg("%s: go to bootloader, lowering output_enable"
				" for a while...\n", __func__);
			spi_set_output_enable(privdata, 0);
			msleep(500);
			spi_set_output_enable(privdata, 1);
			ntrig_dbg("%s: go to bootloader, output_enable is back "
				"up\n", __func__);
		}
...
static int register_to_dispatcher(struct spi_device *spi)
{
	...
	printk("ntrig irq_pin:%d state:%d\n", privdata->irq_gpio, ntrig_get_irq_value());
	printk("ntrig oe_gpio:%d\n", privdata->oe_gpio);
...
static void spi_set_pwr(struct ntrig_spi_privdata *pdata, bool enable)
{
	if (pdata->pwr_gpio >= 0) { /* power gpio is present */
		if (enable) {
			printk(KERN_DEBUG "ntrig power pin:%d 1\n", pdata->pwr_gpio);
			...
			printk(KERN_DEBUG "ntrig power pin:%d 0\n", pdata->pwr_gpio);
			spi_set_output_enable(pdata, enable);
...
static int init_spi_pwr_gpio(struct ntrig_spi_privdata *pdata)
{
	if (pdata->pwr_gpio >= 0) { /* power gpio is present */
		...
		if (err) {
			ntrig_err("%s: fail to request gpio for pwr(%d), "
		...
		printk("ntrig set power gpio to high\n");
		msleep(50);
	}
	return 0;
}
	...
	struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (ch == '0') {
		...
		if (err)
			ntrig_err("%s: fail to request gpio for irq(%d), "
		...
		ntrig_dbg("%s: gpio_irq value is %d\n", __func__, val);
			...
			if (err)
				ntrig_err("%s: fail to request gpio for "
	 ...
	 *               using hid_ncp */
		...
		if (err)
			ntrig_err("%s: register_to_dispatcher failed. "
				"result=%d\n", __func__, err);
...
static int ntrig_spi_suspend(struct spi_device *dev, pm_message_t mesg)
{
	...
	down(&privdata->spi_lock);
	g4_swipe_touch(privdata->sensor_id);
	up(&privdata->spi_lock);
...
static int __devinit g4_ntrig_spi_probe(struct spi_device *spi)
{
	...
	err = gpio_direction_input(irq_gpio);
	if (err) {
		printk("%s: fail to set irq input, err = %d\n",
				__FUNCTION__, err);
	}
...
static int __init g4_ntrig_spi_init(void)
{
	...
	if (ret) {
		printk(KERN_ERR "inside %s: failed to create sysfs_group for "
