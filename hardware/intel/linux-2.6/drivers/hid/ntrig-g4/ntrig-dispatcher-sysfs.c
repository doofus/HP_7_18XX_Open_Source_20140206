...
static void ntrig_create_virtualkeys_file(void)
{
	...
	if (board_properties_kobj) {
		ntrig_dbg("inside %s - virtualkey file for the device already "
			"created !\n", __func__);
		return;
	}
	...
	struct kobj_attribute *attr, char *buf)
{
	...
	if (ATTRIBUTE_NAME_MATCHES(attr, "touch_screen_border")) {
		...
		ntrig_err("ntrig-sysfs-dispatcher: inside %s , unknown "
			"configuration\n", __func__);
		retval = DTRG_FAILED;
	}
	...
	struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (ATTRIBUTE_NAME_MATCHES(attr, "touch_screen_border")) {
		ntrig_dbg("ntrig-sysfs-dispatcher: inside %s, "
			"CONFIG_TOUCH_SCREEN_BORDER, data = %d\n", __func__,
			(int)val);
		retval = g4_write_config_dispatcher(&val,
			CONFIG_TOUCH_SCREEN_BORDER);
	} else if (ATTRIBUTE_NAME_MATCHES(attr, "debug_print")) {
		ntrig_dbg("ntrig-sysfs-dispatcher: inside %s, "
			"CONFIG_DEBUG_PRINT, data = %d\n", __func__, (int)val);
	...
	struct kobj_attribute *attr, char *buf)
{
	...
	*value = debug_server_port;
	ntrig_dbg("ntrig-dispatcher-sysfs: inside %s, debug server port is "
	...
	struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (count < sizeof(int))
		return 0;
	value = *((int *) buf);
	ntrig_dbg("ntrig-dispatcher-sysfs: inside %s, changing debug server "
	...
	struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (buf[0] == '0') {
		...
		ntrig_err("Inside %s: RESET_COUNTERS - wrong data=%d, only "
			"valid data is 0\n", __func__, *buf);
	return ret;
}
