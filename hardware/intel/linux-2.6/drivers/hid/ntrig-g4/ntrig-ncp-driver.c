	...
	loff_t *offp)
{
	...
	if (!memory_lock(&g4_g_sem_read)) {	/* failed to lock */
		ntrig_err("ntrig-ncp-driver inside %s -> "
	...
	if (retval < 0) {
		ntrig_err("ntrig-ncp-driver: Leaving %s, "
	...
	if (res) {
		retval = DTRG_FAILED;
		ntrig_err("ntrig-ncp-driver: Leaving %s, failed to "
...
{
	char *in_buf = NULL;
	ntrig_dbg("ntrig-ncp-driver inside %s filep=%p,buff=%p,count=%d,"
	...
	if (copy_from_user(in_buf, buff, count) != 0) {
		ntrig_err("ntrig-ncp-driver Userspace -> "
			"kernel copy failed!\n");
		goto FREE_ERR;
	}
...
int g4_bus_init_ncp(void)
{
	...
	if (g4_setup_ncp(&g4_read_ncp_message, &g4_dispatch_ncp_message)) {
		ntrig_err("ntrig-ncp-driver inside %s cannot start spi "
			"dispatcher\n", __func__);
		goto ERROR;
	}

	if (register_chrdev(g_major_ncp, DEVICE_NAME_NCP, &ncp_fops)) {
		ntrig_err("ntrig-ncp-driver inside %s failed to register "
			"driver\n", __func__);
		goto ERROR_START;
	}
