...
static int my_open(struct inode *inode, struct file *filep)
{
	...
	memory_unlock(&flag);
	if (!accept) {
		ntrig_err("ntrig-direct-event-driver inside %s: concurrent "
			"sessions unsupported - rejecting\n", __func__);
		return -EBUSY;
	}
	ntrig_dbg("ntrig-direct-event-driver inside %s, file mode=%d, "
	...
	loff_t *offp)
{
	...
	memory_unlock(&flag);
	if (interrupted) {
		ntrig_dbg("ntrig-direct-event-driver inside %s: read "
	...
	if (ready) {
		if (copy_to_user(buff, &data, sizeof(data)) != 0) {
			ntrig_err("ntrig-direct-event-driver: kernel -> "
...
int g4_bus_init_direct_events(void)
{
	...
	if (g4_setup_direct_events(on_message)) {
		ntrig_err("ntrig-direct-event-driver inside %s cannot start "
			"dispatcher\n", __func__);
	...
	if (res < 0) {
		printk(KERN_ERR "%s: Failed to add direct-event device, "
			"error %d", __func__, res);
