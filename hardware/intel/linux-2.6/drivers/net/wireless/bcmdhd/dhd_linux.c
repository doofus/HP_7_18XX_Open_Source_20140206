...
/* to enable reload firmware in a kernel thread Xinrui */
#ifndef DHD_REOPEN
#define DHD_REOPEN 1
#endif
...
uint is_dhd_stop = 1; // add by jack
...
dhd_ioctl_entry(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	if (dev_close_flag) {
		printf("%s: dhd is closed. skip it.\n", __func__);
		return -ENODEV;
	}
	DHD_OS_WAKE_LOCK(&dhd->pub);
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	/* If the default firmware path "fw_path" was defined by kernel config
	  * we should update it still in order to identify the different chip version automatically*/
	if ((firmware_path != NULL) && (firmware_path[0] != '\0')){
		COPY_FW_PATH_BY_CHIP(bus, fw_path, firmware_path);
	}else if ((fw_path != NULL) && (fw_path[0] != '\0')){
		COPY_FW_PATH_BY_CHIP(bus, fw_path, fw_path);
	}
	...
	if(!(net=alloc_netdev(sizeof(dhd),"wlan%d",ether_setup))){
		DHD_ERROR(("%s: OOM - alloc_etherdev\n", __FUNCTION__));
		goto fail;
	}
...
#endif 
extern int dhd_bus_poll_control(dhd_pub_t *dhdp);
/*
