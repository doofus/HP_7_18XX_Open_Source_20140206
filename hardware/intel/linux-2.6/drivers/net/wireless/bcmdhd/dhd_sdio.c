...
extern void bcmsdh_oob_irq_manual();
int
dhd_bus_rxctl(struct dhd_bus *bus, uchar *msg, uint msglen)
{
	...
	if (bus->dhd->rxcnt_timeout) {
            bcmsdh_oob_irq_manual();
        }
retry_pending:
	/* Wait until control frame is available */
	timeleft = dhd_os_ioctl_resp_wait(bus->dhd, &bus->rxlen, &pending);
	
	if((pending == TRUE) && (!retry)) {
		retry = 1;
		osl_delay(500);
		goto retry_pending;
	}
...
dhdsdio_isr(void *arg)
{
	...
	if (bus->sleeping || dhd_mmc_suspend) {
...
int dhd_bus_poll_control(dhd_pub_t *dhdp)
{
	dhdp->bus->poll = FALSE;
}
