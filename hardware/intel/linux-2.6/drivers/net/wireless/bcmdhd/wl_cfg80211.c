	...
	u8 *mac, struct station_info *sinfo)
{
	...
	if (wl_get_mode_by_netdev(wl, dev) == WL_MODE_AP) {
		...
		if(sta->idle >= 300){
		   sta->idle = 150;
		   WL_ERR(("@@@@@@@@@@ DHD found station idle timeout @@@@@@@@@@\n"));
		}
...
static s32 wl_inform_single_bss(struct wl_priv *wl, struct wl_bss_info *bi)
{
	...
	if ((notif_bss_info->channel > CH_MAX_2G_CHANNEL) && (notif_bss_info->channel) < 20){
		printk("%s: @@@@@ Got error channel,skip now, channel = %d \n", __func__, (notif_bss_info->channel));
		return err;
	}
	...
	const wl_event_msg_t *e, void *data, bool completed)
{
	...
	if (wl_get_drv_status(wl, CONNECTING, ndev)) {
		...
		if (completed) {
			WL_ERR((" wl_get_assoc_ies"));		
			wl_get_assoc_ies(wl, ndev);
			WL_ERR((" wl_update_prof"));
			wl_update_prof(wl, ndev, NULL, (void *)(e->addr.octet), WL_PROF_BSSID);
			curbssid = wl_read_prof(wl, ndev, WL_PROF_BSSID);
			WL_ERR((" wl_update_bss_info"));
			
			wl_update_bss_info(wl, ndev);
			WL_ERR((" wl_update_pmklist"));
			wl_update_pmklist(ndev, wl->pmk_list, err);
			WL_ERR((" wl_set_drv_status CONNECTED"));
			wl_set_drv_status(wl, CONNECTED, ndev);
		}
		WL_ERR((" cfg80211_connect_result\n"));
...
extern is_dhd_stop; //add by jack
void
