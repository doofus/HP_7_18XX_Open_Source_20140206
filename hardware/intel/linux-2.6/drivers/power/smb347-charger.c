...
{
	int ret, status;
	static int prev_status = POWER_SUPPLY_STATUS_DISCHARGING;
...
cache_status_info:
	/*
	 * show status as full b/w full to charging
	 * and charging full trantisions.
	 */
	if ((prev_status == POWER_SUPPLY_STATUS_FULL) &&
		(status == POWER_SUPPLY_STATUS_CHARGING))
		status = prev_status;
	else
		prev_status = status;
