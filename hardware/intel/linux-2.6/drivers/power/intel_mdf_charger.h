...
#define MSIC_BATT_CHR_CHRSAFELMT_ADDR	0x191	/* Maximum safe charging
						   voltage and current */
#define MSIC_BATT_CHR_CHRSAFELMT	0x40
/* Status registers */
...
#define MSIC_BTP_ADC_MIN	87
#define MSIC_BTP_ADC_MAX	1020
...
#define MSIC_BATT_TEMP_MAX		45 /* set msic stop charging temperature to 45 Celsius degree */
...
struct msic_batt_props {
	...
	unsigned int not_present_times;
	unsigned int not_present_max_times;
};
...
struct msic_power_module_info {
	...
	/* WA to initiate a forceful disconnect event.
	 * When plug/unplug is done very fast, charger driver is not getting
	 * DISCONNECT event from OTG, but PMIC would report USB-disconnect.
	 * Hence, this worker to run in such case.
	 */
	struct delayed_work force_disconn_dwrk;
