...
static int mt9v113_wait_mipi_standby(struct v4l2_subdev *sd, int flag)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int i = 100;
	int ret, status;
	...
	while (i--) {
		...
		if ((status & MIPI_STAT_MASK_MIPI_STBY_STAT)
		    >> MIPI_STAT_BIT_MIPI_STBY_STAT == flag)
			return 0;
		usleep_range(8000, 12000);
	}
...
static int mt9v113_init_common(struct v4l2_subdev *sd)
{
	 ...
	 * res init
	 */
	ret = mt9v113_write_reg_array(client, mt9v113_res_init);
	if (ret) {
		dev_err(&client->dev, "err res init: %d", ret);
		return ret;
	}
...
/*
 * This is to do the complete re-initliazation for the sensor in case for
 * MIPI standby execution failure.
 */
static int mt9v113_recover(struct v4l2_subdev *sd, int flag)
{
	struct mt9v113_device *dev = to_mt9v113_sensor(sd);
	struct i2c_client *c = v4l2_get_subdevdata(sd);
	int ret;

	dev_warn(&c->dev, "recover flag:%d.\n", flag);

	ret = mt9v113_init_common(sd);
	if (ret)
		return ret;

	/* re-init the resolution table */
	switch (dev->res) {
	case MT9V113_RES_QCIF:
		ret = mt9v113_write_reg_array(c, mt9v113_qcif_init);
		break;
	...
	/* Arbitrary value from Aptina */
	ret = mt9v113_write_reg(c, MISENSOR_16BIT, MT9V113_REG_STBY_CTRL,
			      0x0028);
	if (ret) {
	...
	if (ret)
		return ret;
	ret = mt9v113_wait_mipi_standby(sd, 0);
...
static int mt9v113_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable) {
		...
		ret = mt9v113_wait_mipi_standby(sd, 0);
		if (ret) {
			dev_warn(&c->dev, "wait mipi standby timeout!");
			return mt9v113_recover(sd, 1);
		}
		...
		ret = mt9v113_wait_mipi_standby(sd, 1);
		if (ret) {
			dev_warn(&c->dev, "wait mipi standby timeout!");
			return mt9v113_recover(sd, 0);
		}
