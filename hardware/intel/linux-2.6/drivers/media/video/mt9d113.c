...
static int mt9d113_s_color_effect(struct v4l2_subdev *sd, int effect)
{
	...
	/* Write col effect register B */
	ret = mt9d113_write_reg(client, MISENSOR_16BIT,
				MT9D113_MCU_VAR_ADDR,
				MT9D113_VAR_COL_EFF_B);
	if (ret) {
		dev_err(&client->dev, "err Write VAR ADDR: %d", ret);
		return ret;
	}
	...
	ret = mt9d113_write_reg(client, MISENSOR_16BIT,
				...
				reg_val);
	if (ret) {
		dev_err(&client->dev, "err read COL_EFF_B: %d", ret);
		return ret;
	}
...
static int mt9d113_context_switch(struct v4l2_subdev *sd, int val)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int ret;
	...
	ret = mt9d113_write_reg(client, MISENSOR_16BIT, MT9D113_MCU_VAR_DATA0,
				 val);
	if (ret) {
		dev_err(&client->dev, "err Write VAR DATA: %d", ret);
		return ret;
	}

	return mt9d113_wait_refresh(sd);
}
...
static int mt9d113_init_common(struct v4l2_subdev *sd)
{
	...
	/*
	 * Set Seq_cap_mode_video = 1
	 * the sequencer does not return to Preview mode until preview mode is
	 * selected for switch
	 */
	ret = mt9d113_write_reg(client, MISENSOR_16BIT, MT9D113_MCU_VAR_ADDR,
				MT9D113_VAR_SEQ_CAP_MODE);
	if (ret) {
		dev_err(&client->dev, "err Write VAR ADDR: %d", ret);
		return ret;
	}
	...
	ret = mt9d113_write_reg(client, MISENSOR_16BIT, MT9D113_MCU_VAR_DATA0,
				 SEQ_CMD_CAP_MODE_VIDEO);
	if (ret) {
		dev_err(&client->dev, "err set seq cap mode: %d", ret);
		return ret;
	}
...
static bool check_update_res_config(struct mt9d113_device *dev, int res)
{
	return (dev->run_mode == CI_MODE_STILL_CAPTURE &&
	    dev->res_contextB != res) ||
	    (dev->run_mode != CI_MODE_STILL_CAPTURE &&
	     dev->res_contextA != res);
}
			      ...
			      struct v4l2_mbus_framefmt *fmt)
{
	...
	bool capture_flag = (dev->run_mode == CI_MODE_STILL_CAPTURE);
	const struct misensor_reg *reglist;
	int ret;
	...
	if (check_update_res_config(dev, res_index->res) == false)
		goto done;

	switch (res_index->res) {
	case MT9D113_RES_QCIF:
		reglist = capture_flag ? mt9d113_qcif_30_init_B :
		    mt9d113_qcif_30_init_A;
		dev_info(&c->dev, "%s: set for qcif\n", __func__);
		break;
	case MT9D113_RES_QVGA:
		reglist = capture_flag ? mt9d113_qvga_30_init_B :
		    mt9d113_qvga_30_init_A;
		dev_info(&c->dev, "%s: set for qvga\n", __func__);
		break;
	case MT9D113_RES_CIF:
		reglist = capture_flag ? mt9d113_cif_30_init_B :
		    mt9d113_cif_30_init_A;
		dev_info(&c->dev, "%s: set for cif\n", __func__);
		break;
	case MT9D113_RES_VGA_WIDE:
		reglist = capture_flag ? mt9d113_vga_wide_29_init_B :
		    mt9d113_vga_wide_29_init_A;
		dev_info(&c->dev, "%s: set for vga wide\n", __func__);
		break;
	case MT9D113_RES_VGA:
		reglist = capture_flag ? mt9d113_vga_30_init_B :
		    mt9d113_vga_30_init_A;
		dev_info(&c->dev, "%s: set for vga\n", __func__);
		break;
	case MT9D113_RES_SVGA:
		reglist = capture_flag ? mt9d113_svga_30_init_B :
		    mt9d113_svga_30_init_A;
		dev_info(&c->dev, "%s: set for svga\n", __func__);
		break;
	case MT9D113_RES_720P:
		reglist = capture_flag ? mt9d113_720p_29_init_B :
		    mt9d113_720p_29_init_A;
		dev_info(&c->dev, "%s: set for 720p\n", __func__);
		break;
	case MT9D113_RES_1M:
		reglist = capture_flag ? mt9d113_1m_15_init_B :
		    mt9d113_1m_15_init_A;
		dev_info(&c->dev, "%s: set for 1m\n", __func__);
		break;
	case MT9D113_RES_2M:
		reglist = capture_flag ? mt9d113_2m_15_init_B :
		    mt9d113_2m_15_init_A;
	...
	ret = mt9d113_write_reg_array(c, reglist);
	if (ret)
		return ret;
done:
	/*
	...
	/* Mode switch */
	if (dev->run_mode == CI_MODE_STILL_CAPTURE) { /* Switch Context B */
		dev->res_contextB = res_index->res;
		ret = mt9d113_context_switch(sd, SEQ_CMD_GOTO_CAPTURE);
		if (ret) {
			dev_err(&c->dev, "context failed: %d", ret);
			return ret;
		}
	} else { /* switch to context A */
		dev->res_contextA = res_index->res;
		ret = mt9d113_context_switch(sd, SEQ_CMD_GOTO_PREVIEW);
		if (ret) {
			dev_err(&c->dev, "context failed: %d", ret);
			return ret;
		}
