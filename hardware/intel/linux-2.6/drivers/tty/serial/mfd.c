...
//pjn add for gps @20130318
struct hsu_port * get_hsu_p(void)
{
	return phsu;
}
//end
 ...
 * For Broadcom BCM43XX device, low latency can be disabled
 * **************************************/
static unsigned int hsu_low_latency = 0;
...
/* add by hunter */
	dev_err(dev, "@@@@@@@@ harry HSU wake up, set pm_flags now");

	if (up->index == 0) {
		dev_err(dev, "@@@@@@@@ harry HSU wake up, set pm_flags now");
		/*Brcm BCM43xx device runtime_idle timeout need longer*/
		if(up->running)
			set_bit(PM_WAKEUP, &up->pm_flags);
	}
...
static int hsu_runtime_idle(struct device *dev)
{
	...
	if (system_state == SYSTEM_BOOTING) {
		...
		{	
			if (up->suspended) {
				/*BCM43xx device need to set longer*/
				if (test_bit(PM_WAKEUP, &up->pm_flags))
				delay = 1000;/*1.5s will be enough for resume*/
				else
				delay = 500;
			} else {
				if (test_bit(PM_WAKEUP, &up->pm_flags))
				delay = 800;/*800ms will be enough for runtime timeout*/
				else
				delay = 400;
			}
		}
...
static int hsu_runtime_suspend(struct device *dev)
{
	...
	if (up->index == 0) /*we always clear this bit*/
		clear_bit(PM_WAKEUP, &up->pm_flags);
	return 0;
}
