 ...
 * 1.00A Jun-30-2011  Tracy   Made a new function
 *****************************************************************************/
int bh1721_driver_init(struct i2c_client *client)
{
	int result;
...
static struct device_attribute attributes[] = {
	__ATTR(enable_als,0x666,attr_get_enable_als,attr_set_enable_als)
};
...
static int rohm_ls_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = gpio_request(GPIO63_ALS_DVI_PIN, "als_int");
	if(ret<0)
		printk("%s,gpio63 request fail,ret:%d\n",ret);
	*/
	gpio_direction_output(GPIO63_ALS_DVI_PIN, 0);
	gpio_set_value_cansleep(GPIO63_ALS_DVI_PIN, 0);
	msleep(1);
	gpio_direction_output(GPIO63_ALS_DVI_PIN, 1);
	gpio_set_value_cansleep(GPIO63_ALS_DVI_PIN, 1);
	
	rohm_sensor_input_init(ls);
	bh1721_driver_init(ls->client);
