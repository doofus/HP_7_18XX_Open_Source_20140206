/* 
 * drivers/input/touchscreen/ft5306_touch.c
 *
...
#define __FT5306_FT__
static struct i2c_client *this_client;
static struct ft5306_touch_platform_data *ft5306_touch_pdata;
unsigned char fts_ctpm_get_upg_ver(void);
static u8 ft5306_tp_type_id; //add for save tp id by liukai
...
//register address
#define FT5306_REG_FW_VER 0xA6

static void ft5306_chip_reset(void)
{

	/* FT5316 wants 40mSec minimum after reset to get organized */
	gpio_set_value(ft5306_touch_pdata->reset, 1);
	msleep(40);
	gpio_set_value(ft5306_touch_pdata->reset, 0);
	msleep(40);
	gpio_set_value(ft5306_touch_pdata->reset, 1);
	msleep(40);
...
/*get the tp's id*/
static void ft5306_tp_id(void)
{
	if(ft5306_read_reg(0xa8, &ft5306_tp_type_id) < 0)
	{
		pr_err("%s ERROR: could not read register\n", __FUNCTION__);
	}
	else 
	{
		if (ft5306_tp_type_id == 0xA8)
		{
			printk("[%s]: Read TP MODULE ID error!\n", __func__);
		}
		else
		{
			printk("[%s]: TP MODULE ID is 0x%02X !\n", __func__, ft5306_tp_type_id);
		}
	}
}
...
E_UPGRADE_ERR_TYPE  fts_ctpm_fw_upgrade(FTS_BYTE* pbt_buf, FTS_DWRD dw_lenth)
{
	...
	byte_read(reg_val,2);
	if( (reg_val[0] == 0x79 && reg_val[1] == 0x07)||(reg_val[0] == 0x79 && reg_val[1] == 0x03))
	{
...
***********************************************/
static ssize_t ft5306_tpfwver_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct ft5306_touch_data *data = NULL;
	DBUG4(printk("[%s]: Enter!\n", __func__);)
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
	data = (struct ft5306_touch_data *) i2c_get_clientdata( client );
	ssize_t num_read_chars = 0;
	u8	   fwver = 0;
	mutex_lock(&data->device_mode_mutex);
	if(ft5306_read_reg(FT5306_REG_FW_VER, &fwver) < 0)
...
static ssize_t ft5306_tpfwver_store(struct device *dev,
...
************************************/
static ssize_t ft5306_tprwreg_show(struct device *dev,	struct device_attribute *attr, char *buf)
{
	DBUG(printk("[%s]: Enter!\n", __func__);)
	/* place holder for future use */
	return -EPERM;
...
static ssize_t ft5306_tprwreg_store(struct device *dev,	struct device_attribute *attr, const char *buf, size_t count)
{
	struct ft5306_touch_data *data = NULL;
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
	data = (struct ft5306_touch_data *) i2c_get_clientdata( client );
	...
	if(2 == num_read_chars)
	{
		...
		regaddr = wmreg;
		if(ft5306_read_reg(regaddr, &regvalue) < 0)
	...
	else
	{
		regaddr = wmreg>>8;
		regvalue = wmreg;
		if(ft5306_write_reg(regaddr, regvalue) < 0)
...
static int ft5306_GetFirmwareSize(char * firmware_name)
{
...
static int ft5306_ReadFirmware(char * firmware_name, unsigned char * firmware_buf)
{
	struct file* pfile = NULL;
	struct inode *inode;
	unsigned long magic; 
...
int fts_ctpm_fw_upgrade_with_app_file(char * firmware_name)
{
	...
	u8 fwver;
	DBUG(printk("[%s]: Enter!\n", __func__);)
	int fwsize = ft5306_GetFirmwareSize(firmware_name);
	if(fwsize <= 0)
	{
	...
	if(ft5306_ReadFirmware(firmware_name, pbt_buf))
	{
...
static ssize_t ft5306_fwupgradeapp_show(struct device *dev,	struct device_attribute *attr, char *buf)
{
	DBUG(printk("[%s]: Enter!\n", __func__);)
	/* place holder for future use */
	return -EPERM;
...
static ssize_t ft5306_fwupgradeapp_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, 
					size_t count)
{
...
#if CFG_SUPPORT_AUTO_UPG
static int ft5306_fwupgrade_with_ifile(void)
{
   FTS_BYTE*     pbt_buf = FTS_NULL;
   FTS_DWRD dw_lenth = 0;
   int i_ret;
	...
	//=========FW upgrade========================*/
	if (ft5306_tp_type_id == 0x51)
	{
		pbt_buf = CTPM_FW_0X51;
		dw_lenth =sizeof(CTPM_FW_0X51);
	}
	else if (ft5306_tp_type_id == 0x53)
	{
		pbt_buf = CTPM_FW_0X53;
		dw_lenth =sizeof(CTPM_FW_0X51);
	}
	else if (ft5306_tp_type_id == 0x79)
	{
	...
	{
		printk(KERN_ERR"ft5306_fwupgrade_ifile(),not support this tp.\n");
		return 1;
	}
	...
	if (i_ret != 0)
	{
		printk(KERN_ERR"ft5306_fwupgrade_ifile() = %d.\n", i_ret);
		//error handling ...
		//TBD
	}
	else
	{
		printk(KERN_ERR"ft5306_fwupgrade_ifile()upgrade successfully.\n");
		fts_ctpm_auto_clb();  //start auto CLB
	}
...
unsigned char ft5306_get_ifile_ver(void)
{
	FTS_BYTE*     pbt_buf = FTS_NULL;
	FTS_DWRD dw_lenth = 0;
	...
	{
		printk(KERN_ERR"ft5306_get_ifile_ver(),not support this tp.\n");
		return 0xff;
	}	
...
int ft5306_fwupgrate_ifile(void)
{
    unsigned char old_ver;
    unsigned char new_ver;
    int           i_ret;

    old_ver = ft5306_read_fw_ver();
    new_ver = ft5306_get_ifile_ver();

	printk(KERN_ERR"ft5306_fwupgrate_ifile():old_ver = 0x%x, new_ver = 0x%x\n",
		old_ver, 
		new_ver);
	...
	//if (old_ver < new_ver)  
	{
		...
		i_ret = ft5306_fwupgrade_with_ifile();   

		if (i_ret == 0)
		{
			msleep(300);
			new_ver = ft5306_read_fw_ver();
			printk(KERN_ERR"ft5306_fwupgrate_ifilenew_ver = 0x%x\n", new_ver);
			return 1;
		}
		else
		{
...
static ssize_t ft5306_fwupgradeifile_show(struct device *dev,	struct device_attribute *attr, char *buf)
{
	DBUG(printk("[%s]: Enter!\n", __func__);)
	int  irt = 0;
	struct ft5306_touch_data *data = NULL;
	...
	disable_irq(client->irq);
	irt = ft5306_fwupgrate_ifile();
	enable_irq(client->irq);		
	return -EPERM;
}

static ssize_t ft5306_fwupgradeifile_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, 
					size_t count)
{
...
release touch panel	
**************************/
static void ft5306_touch_release(void)
{
	struct ft5306_touch_data *data = i2c_get_clientdata(this_client);
...
static int ft5306_read_data(void)
{
	...
	ret = ft5306_i2c_rxdata(buf, CFG_POINT_READ_BUF);
    if (ret < 0) {
...
static void ft5306_report_value(void)
{
	...
	for (i  = 0; i < event->touch_point; i++)
	{
		if ((event->au16_x[i] < SCREEN_MAX_X)
			&& (event->au16_y[i] < SCREEN_MAX_Y +100))
		{
			/* LCD view area. */
			input_report_abs(data->input_dev,
				...
				event->au16_x[i]);
		#if 0	
			if ((ft5306_tp_type_id == 0x79)&&(event->au16_y[i] >= SCREEN_MAX_Y -1 ))
			{
			input_report_abs(data->input_dev,
				ABS_MT_POSITION_Y,
				event->au16_y[i] + 10);
	...
	if (event->touch_point == 0)
		ft5306_touch_release();
}

static void ft5306_touch_pen_irq_work(struct work_struct *work)
{
	int ret = -1;
	
	if (ft5306_read_data()== 0)	
	{	
		ft5306_report_value();
	}
	else
...
static irqreturn_t ft5306_touch_interrupt(int irq, void *dev_id)
{
	struct ft5306_touch_data *ft5306_touch = dev_id;
	DBUG(printk("[%s]: Enter!\n",__func__);)
	if (!work_pending(&ft5306_touch->pen_event_work)) {
		queue_work(ft5306_touch->ts_workqueue, &ft5306_touch->pen_event_work);
	}
...
static void ft5306_touch_suspend(struct early_suspend *handler)
{
	struct ft5306_touch_data *ts;
	ts =  container_of(handler, struct ft5306_touch_data, early_suspend);
	...
	if( event->touch_point != 0) 
	{
		printk("[%s]:Find the touchpoint when ft5306 suspend!\n", __func__);
		event->touch_point = 0;
		ft5306_touch_release();
	}
	/*add end*/
...
static void ft5306_touch_resume(struct early_suspend *handler)
{
	printk("[%s]: Enter!\n", __func__);
		/*reset touch*/
	ft5306_chip_reset();
...
/* sysfs */
/*read the version of firmware*/
static DEVICE_ATTR(fwversion, S_IRUGO|S_IWUSR, ft5306_tpfwver_show, ft5306_tpfwver_store);
/*read and set the report rate*/
static DEVICE_ATTR(fwrate, S_IRUGO|S_IWUSR, ft5306_tprwreg_show, ft5306_tprwreg_store);
/*upgrade the tp firmware with app.bin*/ 
static DEVICE_ATTR(fwupdate, S_IRUGO|S_IWUSR, ft5306_fwupgradeapp_show, ft5306_fwupgradeapp_store);

#if CFG_SUPPORT_AUTO_UPG
static DEVICE_ATTR(fwupdateifile, S_IRUGO|S_IWUSR, ft5306_fwupgradeifile_show, ft5306_fwupgradeifile_store);
#endif
...
static struct attribute_group ft5306_attribute_group = {
...
int ft5306_ft_i2c_Read(char *writebuf,
...
/*write data by i2c*/
int ft5306_ft_i2c_Write(char *writebuf, int writelen)
{
	int ret;
...
int ft5306_ft_write_reg(u8 regaddr, u8 regvalue)
{
	unsigned char buf[2] = {0};
	buf[0] = regaddr;
	buf[1] = regvalue;

	return ft5306_ft_i2c_Write(buf, sizeof(buf));
}


int ft5306_ft_read_reg(u8 regaddr, u8 *regvalue)
{
	return ft5306_ft_i2c_Read(&regaddr, 1, regvalue, 1);
}
static ssize_t ft5306_vendor_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	ssize_t num_read_chars = 0;
	...
	/*chip id*/
	if(ft5306_read_reg(0xa8, &id_type) < 0)
	{
...
static ssize_t ft5306_vendor_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	/*place holder for future use*/
...
static ssize_t ft5306_ft_tprwreg_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	/*place holder for future use*/
...
static ssize_t ft5306_ft_tprwreg_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	//struct i2c_client *client = container_of(dev, struct i2c_client, dev);
	...
	if (2 == num_read_chars) {
		/*read register*/
		regaddr = wmreg;
		if (ft5306_ft_read_reg(regaddr, &regvalue) < 0)
		...
		regaddr = wmreg >> 8;
		regvalue = wmreg;
		if (ft5306_ft_write_reg(regaddr, regvalue) < 0)
...
static int ft5306_ft_read_rawdata(u16 rawdata[][FTS_RX_MAX],
			u8 tx, u8 rx)
{
	u8 i = 0, j = 0, k = 0;
	int err = 0;
	...
	/*scan*/
	err = ft5306_ft_read_reg(FTS_DEVICE_MODE_REG, &regvalue);
	if (err < 0) {
		return err;
	} else {
		regvalue |= 0x80;
		err = ft5306_ft_write_reg(FTS_DEVICE_MODE_REG, regvalue);
		if (err < 0) {
			return err;
		} else {
			for(i=0; i<20; i++)
			{
				msleep(8);
				err = ft5306_ft_read_reg(FTS_DEVICE_MODE_REG, 
							&regvalue);
				if (err < 0) {
					return err;
				} else {
	...
	for(i=0; i<tx; i++)
	{
		...
		writebuf[1] = i;
		err = ft5306_ft_i2c_Write(writebuf, 2);
		if (err < 0) {
			return err;
		}
		/* Read the data for this row */
		regaddr = FTS_RAW_BEGIN_REG;
		err = ft5306_ft_i2c_Read( &regaddr, 1, read_buffer, rx*2);
		if (err < 0) {
			return err;
		}
		k = 0;
...
/*raw data show*/
static ssize_t ft5306_ft_rawdata_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	ssize_t num_read_chars = 0;
	...
	//mutex_lock(&g_device_mutex);
	/*entry factory*/
	err = ft5306_ft_write_reg(FTS_DEVICE_MODE_REG, FTS_FACTORYMODE_VALUE);
	if (err < 0) 
	{
	...
	/*get rx and tx num*/
	err = ft5306_ft_read_reg(FTS_TXNUM_REG, &tx);
	if (err < 0) 
	{
		num_read_chars = sprintf(buf,"%s:get tx error!\n", __func__);
		goto RAW_ERROR;
	}
	err = ft5306_ft_read_reg(FTS_RXNUM_REG, &rx);
	if (err < 0)
	{
	...
	num_read_chars += sprintf(&(buf[num_read_chars]),"FT5306 tp channel: %u * %u\n", tx, rx);
	num_read_chars += sprintf(&(buf[num_read_chars]),"Reference data:\n");

	/*get rawdata*/
	err = ft5306_ft_read_rawdata(before_rawdata, tx, rx);
	if (err < 0) {
		num_read_chars = sprintf(buf,
...
RAW_ERROR:
	/*enter work mode*/
	err = ft5306_ft_write_reg(FTS_DEVICE_MODE_REG, FTS_WORKMODE_VALUE);
	if (err < 0)
...
static ssize_t ft5306_ft_rawdata_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	/*place holder for future use*/
...
static ssize_t ft5306_ft_diffdata_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	ssize_t num_read_chars = 0;
	...
	err = ft5306_ft_read_reg(FTS_VOLTAGE_REG, &regvalue);
	if (err < 0) 
	{
	...
	num_read_chars += sprintf(&(buf[num_read_chars]),
		...
		orig_vol, regvalue);
	#endif
	err = ft5306_ft_write_reg(FTS_VOLTAGE_REG, regvalue);
	if (err < 0) 
	{
	...
	/*get rawdata*/
	for (i=0; i<3; i++)
		err = ft5306_ft_read_rawdata(after_rawdata, tx, rx);
	if (err < 0) {
		num_read_chars = sprintf(buf,"%s:diffdata show error!\n", __func__);
...
RETURN_ORIG_VOLTAGE:
	err = ft5306_ft_write_reg(FTS_VOLTAGE_REG, orig_vol);
	if (err < 0)
...
static ssize_t ft5306_ft_diffdata_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	/*place holder for future use*/
...
static ssize_t ft5306_ft_diag_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	...
	num_read_chars1 = ft5306_ft_rawdata_show(dev,attr,buf);
	num_read_chars2 = ft5306_ft_diffdata_show(dev,attr,(buf+num_read_chars1));
	return (num_read_chars1+ num_read_chars2);
}
static ssize_t ft5306_ft_diag_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	/*place holder for future use*/
...
static void ft5306_ft_OnAnalyseRXOpenShort(u8 tx, u8 rx,
...
static  void ft5306_ft_OnAnalyseTXOpenShort(u8 tx, u8 rx,
...
/*open short show*/
static ssize_t ft5306_ft_openshort_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	ssize_t num_read_chars = 0;
	...
	num_read_chars += sprintf(&(buf[num_read_chars]),"tp channel: %u * %u\n", tx, rx);
	/*get rawdata*/
	err = ft5306_ft_read_rawdata(rawdata, tx, rx);
	if (err < 0) 
	{
		num_read_chars = sprintf(buf,"%s:diffdata show error!\n", __func__);
		goto RAW_ERROR;
	...
	/*get rawdata*/
	for (i=0; i<3; i++)
		err = ft5306_ft_read_rawdata(diffdata, tx, rx);
	if (err < 0) {
		num_read_chars = sprintf(buf,"%s:diffdata show error!\n", __func__);
		goto RETURN_ORIG_VOLTAGE;
	} else {
	...
	if (0 == bpass) 
	{
		ft5306_ft_OnAnalyseRXOpenShort(tx, rx, diffdata, rawdata, buf, &num_read_chars);
		ft5306_ft_OnAnalyseTXOpenShort(tx, rx, diffdata, rawdata, buf, &num_read_chars);
	} else
		num_read_chars = sprintf(buf,
...
static ssize_t ft5306_ft_openshort_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	/*place holder for future use*/
...
static ssize_t ft5306_ft_setrawrange_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	ssize_t num_read_chars = 0;
...
static ssize_t ft5306_ft_setrawrange_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	//struct i2c_client *client = container_of(dev, struct i2c_client, dev);
...
static ssize_t ft5306_ft_setdiffrange_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	ssize_t num_read_chars = 0;
...
static ssize_t ft5306_ft_setdiffrange_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	//struct i2c_client *client = container_of(dev, struct i2c_client, dev);
...
static ssize_t ft5306_ft_setvoltagelevel_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	ssize_t num_read_chars = 0;
...
static ssize_t ft5306_ft_setvoltagelevel_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
//	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
...
*example:cat version
-------------------------------------------------------------------------*/
//static DEVICE_ATTR(version, (S_IWUSR|S_IRUGO), ft5306_version_show,
//			ft5306_version_store);
...
*example:cat id
-------------------------------------------------------------------------*/
static DEVICE_ATTR(vendor, (S_IWUSR|S_IRUGO), ft5306_vendor_show,
			ft5306_vendor_store);
...
-------------------------------------------------------------------------*/
static DEVICE_ATTR(rwreg, S_IRUGO | S_IWUSR, ft5306_ft_tprwreg_show,
			ft5306_ft_tprwreg_store);

/*------------------------------------------------------------------------
*show a frame rawdata
*example:example:cat rawdata
-------------------------------------------------------------------------*/
//static DEVICE_ATTR(rawdata, S_IRUGO | S_IWUSR, ft5306_ft_rawdata_show,
//			ft5306_ft_rawdata_store);

/*------------------------------------------------------------------------
*show a frame diffdata
*example:cat diffdata
-------------------------------------------------------------------------*/
//static DEVICE_ATTR(diffdata, S_IRUGO | S_IWUSR, ft5306_ft_diffdata_show,
//			ft5306_ft_diffdata_store);
...
*example:cat diag
-------------------------------------------------------------------------*/
static DEVICE_ATTR(diag, S_IRUGO | S_IWUSR, ft5306_ft_diag_show,
			ft5306_ft_diag_store);
...
-------------------------------------------------------------------------*/
static DEVICE_ATTR(openshort, S_IRUGO | S_IWUSR, ft5306_ft_openshort_show,
			ft5306_ft_openshort_store);
...
*		cat setraw
-------------------------------------------------------------------------*/
static DEVICE_ATTR(setraw, S_IRUGO | S_IWUSR, ft5306_ft_setrawrange_show,
			ft5306_ft_setrawrange_store);
/*------------------------------------------------------------------------
...
static DEVICE_ATTR(setdiffdata, S_IRUGO | S_IWUSR, ft5306_ft_setdiffrange_show,
			ft5306_ft_setdiffrange_store);
/*------------------------------------------------------------------------
*set range of diffdata
...
static DEVICE_ATTR(setvol, S_IRUGO | S_IWUSR,
			ft5306_ft_setvoltagelevel_show,
			ft5306_ft_setvoltagelevel_store);




static struct kobject *android_touch_ftobj;
static int ft5306_touch_sysfs_init(void)
{
	int ret;
...
static void ft5306_touch_sysfs_deinit(void)
{
...
static ssize_t ft5306_virtual_keys_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	if (ft5306_tp_type_id == 0x51)
	{
		return sprintf(buf, 
...
static int ft5306_virtual_keys_init(void) 
{
	
	int ret;
    //start@duyichun modify for kobject create error
	properties_kobj = intel_get_properties();
	if (!properties_kobj)
	{
...
static int ft5306_touch_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	struct ft5306_touch_data *ft5306_touch;
	struct input_dev *input_dev;
	int err = 0;
	unsigned char uc_reg_value; 
	
	ft5306_touch_pdata = client->dev.platform_data;
	if (ft5306_touch_pdata == NULL) 
	{
	...
	/*init RESET pin*/
	err = gpio_request(ft5306_touch_pdata->reset, "ft5306_chip_reset");
	if (err < 0)
	{
		printk(KERN_ERR "Failed to request GPIO%d (ft5306_chip_reset) error=%d\n",
		ft5306_touch_pdata->reset, err);
	}

	err = gpio_direction_output(ft5306_touch_pdata->reset, 1);
	if (err)
	{
		printk(KERN_ERR "Failed to set reset direction, error=%d\n", err);
		gpio_free(ft5306_touch_pdata->reset);
	}
...
/* Check if the I2C bus supports BYTE transfer */
	...
	ft5306_touch = kzalloc(sizeof(*ft5306_touch), GFP_KERNEL);
	if (!ft5306_touch)	
	{
		err = -ENOMEM;
	...
	this_client = client;
	i2c_set_clientdata(client, ft5306_touch);
	mutex_init(&ft5306_touch->device_mode_mutex);
	
	INIT_WORK(&ft5306_touch->pen_event_work, ft5306_touch_pen_irq_work);

	ft5306_touch->ts_workqueue = create_singlethread_workqueue(dev_name(&client->dev));
	if (!ft5306_touch->ts_workqueue) 
	{
		err = -ESRCH;
	...
	/*init INTERRUPT pin*/
	err = gpio_request(ft5306_touch_pdata->irq, 0);
	if(err < 0)
	{
		dev_err(&client->dev,
			    "Failed to request GPIO%d ,error=%d\n",
			    ft5306_touch_pdata->irq,
			    err);	
	}

	err = gpio_direction_input(ft5306_touch_pdata->irq);
	if (err)
	{
		dev_err(&client->dev,
			    "Failed to set interrupt direction, error=%d\n",
			    err);		
		gpio_free(ft5306_touch_pdata->irq);
	}


	
	err = request_irq(gpio_to_irq(ft5306_touch_pdata->irq),
		              ft5306_touch_interrupt, 
	...
	if (err < 0) 
	{
		dev_err(&client->dev, "ft5306_probe: request irq failed\n");
	...
	ft5306_touch->input_dev = input_dev;
	...
	//ft5306_fwupgrate_ifile();

	/*tp's device name*/
	input_dev->name	= "ft5316";		//dev_name(&client->dev)
	input_dev->phys = "ft5316";
	...
	{
		dev_err(&client->dev,
		"ft5306_touch_probe: failed to register input device: %s\n",
		dev_name(&client->dev));
		goto exit_input_register_device_failed;
	}

#ifdef CONFIG_HAS_EARLYSUSPEND
	//ft5306_touch->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1;
	ft5306_touch->early_suspend.level = EARLY_SUSPEND_LEVEL_STOP_DRAWING + 1;
	ft5306_touch->early_suspend.suspend = ft5306_touch_suspend;
	ft5306_touch->early_suspend.resume	= ft5306_touch_resume;
	register_early_suspend(&ft5306_touch->early_suspend);
#endif

	uc_reg_value = ft5306_read_fw_ver();
	printk("[%s]: Firmware version is 0x%x\n", __func__, uc_reg_value);
	
	ft5306_read_reg(FT5306_REG_PERIODACTIVE, &uc_reg_value);
	printk("[%s]: report rate is %dHz.\n", __func__, uc_reg_value * 10);

	if(uc_reg_value != 0xb) 
	{
		ft5306_write_reg(FT5306_REG_PERIODACTIVE, 0xb);
		ft5306_read_reg(FT5306_REG_PERIODACTIVE, &uc_reg_value);
		printk("[%s]: set report rate to %dHz.\n", __func__, uc_reg_value * 10);
	}
	
	ft5306_read_reg(FT5306_REG_THGROUP, &uc_reg_value);
	printk("[%s]: touch threshold is %d.\n", __func__, uc_reg_value * 4);
	
	enable_irq(client->irq);

	//create sysfs
	err = sysfs_create_group(&client->dev.kobj, &ft5306_attribute_group);
	if (0 != err)
	{
		dev_err(&client->dev, "%s() - ERROR: sysfs_create_group() failed: %d\n", __FUNCTION__, err);
		sysfs_remove_group(&client->dev.kobj, &ft5306_attribute_group);
	}
	else
	{
...
exit_input_dev_alloc_failed:
	free_irq(client->irq, ft5306_touch);
exit_irq_request_failed:
exit_platform_data_null:
	cancel_work_sync(&ft5306_touch->pen_event_work);
	destroy_workqueue(ft5306_touch->ts_workqueue);
...
remove touch
***********************************************************************************************/
static int __devexit ft5306_touch_remove(struct i2c_client *client)
{
	DBUG(printk("[%s]: Enter!\n", __func__);)
	struct ft5306_touch_data *ft5306_touch = i2c_get_clientdata(client);
#ifdef __FT5306_FT__		
	ft5306_touch_sysfs_deinit();
#endif	
	unregister_early_suspend(&ft5306_touch->early_suspend);
	free_irq(client->irq, ft5306_touch);
	input_unregister_device(ft5306_touch->input_dev);
	kfree(ft5306_touch);
	cancel_work_sync(&ft5306_touch->pen_event_work);
	destroy_workqueue(ft5306_touch->ts_workqueue);
	i2c_set_clientdata(client, NULL);
...
MODULE_DEVICE_TABLE(i2c, ft5306_touch_id);
/***********************************************************************************************
touch driver
***********************************************************************************************/
static struct i2c_driver ft5306_touch_driver = {
	.probe		= ft5306_touch_probe,
	.remove		= __devexit_p(ft5306_touch_remove),
...
static int __init ft5306_touch_init(void)
{
	int ret;
	DBUG(printk("[%s]: Enter!\n", __func__);)
	ret = i2c_add_driver(&ft5306_touch_driver);
	DBUG(printk("ret=%d\n",ret);)
	return ret;
}
