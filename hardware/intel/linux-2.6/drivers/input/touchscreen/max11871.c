			...
			u16 fw_index)
{
	...
	u16 fw_config_crc16, fw_file_crc16;

	fw_config_crc16 = CONFIG(fw_image[fw_index]).crc16;
	fw_file_crc16 = crc16(0, fw->data, CONFIG(fw_image[fw_index]).length);

	if (fw_file_crc16 != fw_config_crc16) {
		ERROR("Firmware file CRC16 (0x%04x) does not match"
			" CRC16 value in configuration (0x%04x)",
			fw_file_crc16, fw_config_crc16);
		return -1;
	}
